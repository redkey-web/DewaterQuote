...
export class NetoAPI {
  ...
  constructor() {
    ...
    this.initialized = !!(this.apiKey && this.storeDomain);

    if (!this.initialized) {
      console.warn('NETO_API_KEY and NETO_API_USERNAME not set - Neto API features will be unavailable');
    }
  }

  private ensureInitialized() {
    if (!this.initialized) {
      throw new Error('Neto API is not configured. Please set NETO_API_KEY and NETO_API_USERNAME environment variables.');
    }
  }
