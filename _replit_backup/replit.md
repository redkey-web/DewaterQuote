...
**Hero Section**: Full-screen hero with professional industrial product photography showcasing Orbit Couplings, strainers, butterfly valves, and ball valves against a technical blueprint-styled background. Clean layout with centered heading, typewriter tagline, and two industrial-styled CTA buttons. Volume discount bar positioned as a separate scrolling section below the hero with light, semi-transparent styling.
...
**Volume Discount Bar**: Reusable `VolumeDiscountsBar` component displayed at the top of all product pages just below the header. Features:
- Semi-transparent aqua (#39C2D9/20%) background with border
- Scrolling animation that pauses on hover
- Volume discounts: 5% (2-4 items), 10% (5-9 items), 15% (10+ items) with discount percentages in red (red-600/red-400)
- Free delivery message: "FREE DELIVERY to all metro areas across Australia"
- TrendingDown and Truck icons from Lucide React
- Implemented in: ProductDetailPage, ValvesPage, StrainersPage, ClampsCouplingsPage, ExpansionJointsPage, ProductListPage, ProductRangePage, BrandPage

**Datasheet Download Button**: Worksite tape styling with diagonal black/yellow stripes pattern using CSS `repeating-linear-gradient`. Features a yellow label with black border containing the download icon and text, positioned over the striped background. Uses shadow-lg for depth.
...
- **CF8M Flanged Float Valve AS4087 PN16**: Full 316 stainless steel float valve for tank level control. Available in DN65, DN80, DN100 sizes. Includes product video and downloadable datasheet. 9-12 weeks lead time.
