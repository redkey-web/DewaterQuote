...
export default function QuoteCart({ isOpen, onClose, items, onRemoveItem, onSubmitQuote }: QuoteCartProps) {
  const pricedItems = items.filter((item) => item.price !== undefined);
  const unpricedItems = items.filter((item) => item.price === undefined);
  const subtotal = pricedItems.reduce((sum, item) => sum + (item.price || 0), 0);
